    ...
    <div className="flex gap-5 h-[calc(100vh-7rem)]">
      ...
      <div className="flex-1 flex flex-col bg-[#111827] rounded-xl border border-[#1F2937] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1F2937]">
          ...
          <button onClick={reset} className="flex items-center gap-1.5 text-xs text-[#6B7280] hover:text-white transition-colors px-2 py-1 rounded hover:bg-[#1F2937]">
              ...
              <div className={`max-w-2xl ${msg.role === 'user' ? 'order-1' : ''}`}>
                ...
                <div className={`rounded-xl px-4 py-3 ${
                  msg.role === 'user'
                    ...
                    : 'bg-[#0D1117] border border-[#1F2937] text-[#D1D5DB]'
                }`}>
          ...
          {isTyping && activeTools.length > 0 && (
            <div className="flex gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2 px-3 py-2 text-xs text-[#6B7280]">
          ...
          <div className="space-y-2">
            {MCP_TOOLS.map(tool => (
              <div key={tool.name} className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[#D1D5DB] text-[11px] font-medium">{tool.label}</p>
                  <p className="text-[#4B5563] text-[10px]">{tool.description}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className={`flex items-start gap-2 px-3 py-2 rounded-lg border text-xs transition-all ${
      ...
      <div>
        ...
        <span className="text-[#6B7280] ml-1">— {toolCall.description}</span>
        {toolCall.status === 'complete' && toolCall.result && (
          <p className="text-[#9CA3AF] mt-0.5 text-[10px]">↳ {toolCall.result}</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-1.5 text-sm leading-relaxed">
      {lines.map((line, i) => {
        ...
        if (line.startsWith('- ')) {
          return <li key={i} className="text-[#D1D5DB] ml-4 list-disc marker:text-[#FBCE07]">{renderInline(line.slice(2))}</li>
        }
        if (line.match(/^\d+\. /)) {
          return <li key={i} className="text-[#D1D5DB] ml-4 list-decimal marker:text-[#FBCE07]">{renderInline(line.replace(/^\d+\. /, ''))}</li>
        }
...
function TableRow({ line }: { line: string }) {
  ...
  return (
    <div className="flex gap-0 text-[11px] border-b border-[#1F2937]">
      {cells.map((cell, i) => (
        <div key={i} className={`px-2 py-1 flex-1 ${isHeader ? 'text-[#FBCE07] font-medium' : 'text-[#D1D5DB]'}`}>
          {cell.trim()}
        </div>
      ))}
    </div>
