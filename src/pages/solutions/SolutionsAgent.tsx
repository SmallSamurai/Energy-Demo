...
const SUGGESTED = [
  ...
  { label: 'Which solutions qualify for ESOS compliance?', icon: '📑', category: 'Compliance' },
...
4. **EV Fleet Charging** — If you have a fleet, smart charging pays back in 3.4 years and cuts a meaningful slice of Scope 3. Can often be co-funded via OZEV grants.
...
export default function SolutionsAgent() {
  ...
  return (
    <div className="flex gap-5 h-[calc(100vh-7rem)]">
      <div className="flex-1 flex flex-col bg-[#111827] rounded-xl border border-[#1F2937] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1F2937]">
          ...
          <button onClick={() => { setMessages([]); setActiveTools([]) }} className="flex items-center gap-1.5 text-xs text-[#6B7280] hover:text-white px-2 py-1 rounded hover:bg-[#1F2937] transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          {messages.length === 0 && !isTyping && (
            <div className="h-full flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-[#6B7280] text-sm max-w-md leading-relaxed mb-8">
              ...
              <div className="grid grid-cols-2 gap-2 w-full max-w-xl">
                {SUGGESTED.slice(0, 4).map(q => (
                  <button key={q.label} onClick={() => send(q.label)}
                    className="text-left p-3 rounded-lg bg-[#0D1117] border border-[#1F2937] hover:border-[#FBCE07]/30 hover:bg-[#FBCE07]/5 transition-all group">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm">{q.icon}</span>
                      <span className="text-[10px] text-[#6B7280] uppercase tracking-wide">{q.category}</span>
                    </div>
                    <p className="text-[#D1D5DB] text-xs group-hover:text-white transition-colors">{q.label}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-2xl ${msg.role === 'user' ? 'order-1' : ''}`}>
                ...
                <div className={`rounded-xl px-4 py-3 ${msg.role === 'user' ? 'bg-[#FBCE07]/10 border border-[#FBCE07]/20 text-white' : 'bg-[#0D1117] border border-[#1F2937] text-[#D1D5DB]'}`}>
                  <MarkdownText content={msg.content} />
                  <p className="text-[10px] text-[#4B5563] mt-2">{msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </div>
              {msg.role === 'user' && (
        ...
        <div className="px-5 py-4 border-t border-[#1F2937]">
          <div className="flex gap-3">
            <input
              className="flex-1 bg-[#0D1117] border border-[#1F2937] rounded-lg px-4 py-2.5 text-sm text-white placeholder-[#4B5563] focus:outline-none focus:border-[#FBCE07]/40 transition-colors"
          ...
          <p className="text-[10px] text-[#4B5563] mt-2 text-center">MCP-connected · solutions catalogue · impact model · carbon registry · market signals</p>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {MCP_TOOLS.map(t => (
              <div key={t.name} className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[#D1D5DB] text-[11px] font-medium">{t.label}</p>
                  <p className="text-[#4B5563] text-[10px]">{t.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            {SUGGESTED.slice(4).map(q => (
              <button key={q.label} onClick={() => send(q.label)} disabled={isTyping}
                className="w-full text-left text-[10px] text-[#9CA3AF] hover:text-white flex items-start gap-1.5 py-1 px-1.5 rounded hover:bg-[#1F2937] transition-all disabled:opacity-40">
...
function ToolBubble({ tc }: { tc: ToolCall }) {
  return (
    <div className={`flex items-start gap-2 px-3 py-2 rounded-lg border text-xs ${tc.status === 'running' ? 'bg-[#FBCE07]/5 border-[#FBCE07]/20' : 'bg-[#0D1117] border-[#1F2937]'}`}>
      {tc.status === 'running'
      ...
      <div>
        ...
        <span className="text-[#6B7280] ml-1">— {tc.description}</span>
        {tc.status === 'complete' && tc.result && <p className="text-[#9CA3AF] mt-0.5 text-[10px]">↳ {tc.result}</p>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-1.5 text-sm leading-relaxed">
      {content.split('\n').map((line, i) => {
        ...
        if (line.startsWith('- ')) return <li key={i} className="text-[#D1D5DB] ml-4 list-disc marker:text-[#FBCE07]">{renderInline(line.slice(2))}</li>
        if (line.match(/^\d+\. /)) return <li key={i} className="text-[#D1D5DB] ml-4 list-decimal marker:text-[#FBCE07]">{renderInline(line.replace(/^\d+\. /, ''))}</li>
        if (line.trim() === '') return <div key={i} className="h-1" />
        return <p key={i} className="text-[#D1D5DB]">{renderInline(line)}</p>
      })}
    </div>
  )
}
...
function TableRow({ line }: { line: string }) {
  ...
  return (
    <div className="flex gap-0 text-[11px] border-b border-[#1F2937]">
      {cells.map((cell, i) => (
        <div key={i} className="px-2 py-1 flex-1 text-[#D1D5DB]">{cell.trim()}</div>
      ))}
    </div>
  )
}
