            ...
            <div className="space-y-2">
              {solutions.sort((a, b) => b.carbonReduction - a.carbonReduction).map(s => (
                <div key={s.id} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-gray-600 w-20 text-right">{s.carbonReduction} tCO₂</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl p-5 border border-gray-200">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={carbonPathwayData} margin={{ top: 5, right: 10, bottom: 0, left: 10 }}>
              ...
              <XAxis dataKey="year" tick={{ fill: '#4B5563', fontSize: 11 }} tickLine={false} />
              <YAxis tick={{ fill: '#4B5563', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={v => `${(v / 1000).toFixed(1)}kt`} />
        ...
        <div className="bg-white rounded-xl p-5 border border-gray-200">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={stackTimelineData} margin={{ top: 5, right: 10, bottom: 0, left: 10 }}>
              ...
              <XAxis dataKey="year" tick={{ fill: '#4B5563', fontSize: 11 }} tickLine={false} />
              <YAxis tick={{ fill: '#4B5563', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={v => `$${v}k`} />
